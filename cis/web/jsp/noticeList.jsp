	...
	<body>
		...
		$(function(){
			...
			$(".message-list").on('click','.message-item',function(){
				location.href=path+'/notice/goNoticeView/'+$(this).attr('objId');
			});
		});
		var dataObj;
				...
				success:function(r){
					...
					if(r.notices.content.length!=0){
						for(var i=0;i<r.notices.content.length;i++){
							var str = 
							'<div class="message-item '+(r.notices.content[i].readFlag==1?'':'message-unread')+'" objId="'+r.notices.content[i].noticeId+'">'+
							'	<i class="icon-star '+(r.notices.content[i].readFlag==1?'light-blue':'orange2')+'"></i>'+
							'	<span title="点击预览" class="sender">'+
							'		<a href='+path+'/notice/goNoticeView/'+r.notices.content[i].noticeId+'>'+r.notices.content[i].title+'</a>'+
